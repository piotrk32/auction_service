...
public class CustomerAuctionController {
    ...
    @GetMapping("/won")
    @Operation(summary = "Show won auctions", description = "Functionality lets user to show all won auctions")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Successful auction acquisition",
    ...
    public ResponseEntity<Page<AuctionResponseDTO>> getWonAuctions(
            @ModelAttribute WonAuctionRequestDTO wonAuctionRequestDTO) {
        Long customerId = fineGrainServices.getCurrentUserId();
        Page<AuctionResponseDTO> wonAuctions = auctionFacade.getWonAuctions(wonAuctionRequestDTO, customerId);
